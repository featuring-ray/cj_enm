...
export default function DraftInboxPage() {
  ...
  const filteredDeliveries = useMemo(() => {
    if (filterTab === "all") return deliveries;
    const statusMap: Record<string, string> = {
      pending: "대기 중",
      accepted: "수락",
      rejected: "거절",
    };
    return deliveries.filter((d) => d.status === statusMap[filterTab]);
  ...
  return (
    <AccountLinkingGate>
      <PageHeader
        title="협업 템플릿 수신함"
        description="캠페인 담당 MD가 보낸 자동 DM 초안을 확인하고 수락/거절합니다"
        actions={
          <Button variant="outline" size="sm" asChild>
            <Link href="/ontner/dm/auto">
